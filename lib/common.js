...
const TRANSLATIONS = {
  ja: {
    ...
    recipientsHint: "既定の送信先（★）は、右クリック送信時に開く選択ウィンドウで最初からチェックが入った状態になります。各送信先の「編集」から、既定の解除や種類の変更がいつでもできます。",
    formTitleAdd: "送信先を追加",
    formNameLabel: "表示名",
    formNamePlaceholder: "例: 自分（仕事用）",
    formEmailLabel: "メールアドレス",
    formEmailPlaceholder: "example@gmail.com",
    formFieldRequiredMsg: "このフィールドを入力してください。",
    formFieldInvalidEmailMsg: "メールアドレスの形式で入力してください（例: example@gmail.com）。",
    formFieldInvalidUrlMsg: "URLの形式で入力してください（例: https://example.com/...）。",
    formTypeLabel: "種類",
    ...
    screenshotSaved: "スクリーンショットをダウンロードフォルダに保存しました:",
    ...
    imageUrlLabel: "画像URL:",
    imageSaved: "画像をダウンロードフォルダに保存しました:",
    imageMailtoNote: "（mailtoの仕様上、画像の自動添付はできません。お手数ですが、開いたメール作成画面に手動で添付してください）",
    imageGmailNote: "（この画像は自動貼り付けできなかったため、URLのみ記載しています。お手数ですが手動で貼り付けてください）",
    // recipient type (追加)
    typeGas: "GAS自動送信",
    ...
    autoSendHelpGasStep3: "コード内の <code>SHARED_SECRET</code> を、自分だけの合言葉に書き換えます（任意ですが、第三者による悪用防止のため推奨。空のままでも動作しますが、設定する場合は手順7で入力する値と完全に一致させてください）",
    ...
    autoSendHelpGasRedeployTitle: "⚠️ コードを後から書き換えたら、必ず「新しいバージョン」として再デプロイしてください",
    autoSendHelpGasRedeployBody: "SHARED_SECRETを後から変更した場合も含め、コードを保存しただけでは公開中のウェブアプリURLには反映されません。「デプロイ」→「デプロイを管理」→ 対象のデプロイの鉛筆アイコン（編集）→「バージョン」で「新バージョン」を選択→「デプロイ」を押して初めて変更が反映されます（URLは変わりません）。これを忘れると、正しい合言葉を入力しているのに毎回「invalid secret」で失敗し続けます。",
  ...
  en: {
    ...
    recipientsHint: "The default recipient (★) starts pre-checked in the picker window that opens on right-click sending. Click \"Edit\" on any recipient to change or remove the default at any time.",
    formTitleAdd: "Add recipient",
    formNameLabel: "Display name",
    formNamePlaceholder: "e.g. Work email",
    formEmailLabel: "Email address",
    formEmailPlaceholder: "example@gmail.com",
    formFieldRequiredMsg: "Please fill out this field.",
    formFieldInvalidEmailMsg: "Please enter a valid email address (e.g. example@gmail.com).",
    formFieldInvalidUrlMsg: "Please enter a valid URL (e.g. https://example.com/...).",
    formTypeLabel: "Type",
    ...
    imageSaved: "Image saved to Downloads folder:",
    imageMailtoNote: "(mailto does not support automatic file attachments. Please attach the image manually.)",
    imageGmailNote: "(This image couldn't be pasted automatically, so only the URL is included. Please paste it manually.)",
    ...
    gasSecretPlaceholder: "Optional passphrase",
    ...
    autoSendHelpGasStep3: "Change <code>SHARED_SECRET</code> in the code to a passphrase only you know (optional but recommended, to prevent misuse by others; leaving it blank also works, but if you do set one, it must exactly match the value you enter in step 7)",
    ...
    autoSendHelpGasRedeployTitle: "⚠️ If you edit the code later, you must redeploy it as a \"new version\"",
    autoSendHelpGasRedeployBody: "Saving the code alone does not update the live web app URL — this includes changing SHARED_SECRET later. Go to Deploy → Manage deployments → click the pencil (edit) icon on the deployment → set \"Version\" to \"New version\" → click Deploy. Only then does the change actually take effect (the URL itself won't change). If you forget this step, sending will keep failing with \"invalid secret\" even though the passphrase you're entering is correct.",
...
async function detectLang() {
  try {
    ...
    if (userLang && userLang !== "auto") {
      return userLang in TRANSLATIONS ? userLang : "en";
    }
  } catch {
    // storage取得失敗時はnavigatorにフォールバック
  }
  // 自動判定: ブラウザの言語が日本語ならja、それ以外（未対応言語も含む）はすべて英語にフォールバックする
